...
mtf_word8_dict = EOL : map Value [minBound .. maxBound]
...
compress alg content size =
    case alg of
        Huffman ->
            let (!tree, bits) = Huffman.encode (BL.unpack content)
            in runPut $ do
...
decompress path content 
    ...
    = let (size, tree, encoded) = flip runGet content $ do
     ...
     in takeExactly size (Huffman.decode (tree, bsToBits encoded))
    | ".lzw" `isSuffixOf` path
    = let 
